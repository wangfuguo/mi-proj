...
//切换部门的窗口
Ext.define('Miser.main.CurrentDeptChangeWindow', {
	...
	doChangeCurrentDept: function(deptCode){
		...
		var successFun = function(json) {
			window.location = '../login/main.action';
        };
        ...
        miser.requestJsonAjax('../login/loginAction!changeCurrentDept.action', params, successFun, failureFun);
	},
	//部门列表
	deptGridPanel : null,
...
Ext.define('Miser.main.topPanel', {
    ...
    getDeptInfo:function(){
    	...
    	if(this.deptInfo==null){
    		var deptText = '<span style="font-size: 18px; font-weight: bold;color:#F15B23;">'+UserContext.getCurrentUserDept().name+'</span>';
    ...
    onChangeUserDeptSpace:function(){
		...
		if(me.currentDeptChangeWindow == null){
			me.currentDeptChangeWindow = Ext.create('Miser.main.CurrentDeptChangeWindow');
		}
...
Ext.define('Miser.main.MainNav',{
	...
	bodyCls:'ye1-body',
	region : 'west',
	store:Ext.create('Miser.store.login.MenuStore'),
	collapsible : false,
	viewConfig :{
		loadMask: false
	},
			...
			this.queryButton = Ext.create('Ext.button.Button',{ 
		    	...
		    	handler: function(){
		    		...
		    		if(!Ext.isEmpty(queryValue)){
		    	        ...
		    	        //Ajax请求得到所有查询到的节点全路径
		    	        miser.requestJsonAjax('../login/menuAction!queryTreePathForName.action', params, successFun, failureFun);
		    		}
		    	}
		    });
		}
...
Ext.onReady(function(){
	...
	var treePanel = Ext.create('Miser.main.MainNav');
	var topPanel = Ext.create('Miser.main.topPanel');
		...
		treePanel,
        {
            ...
            listeners : {
                ...
                beforeremove : function(tabPanel, newCard){
                	//活动管理页面关闭时清空临时表
                	if(newCard.id == '101002' || newCard.id == '101003'){
                		miser.requestJsonAjax('../discount/priceEvnetAction!deleteCorpImportTemp.action', null, 
            			function(response){
            			}, 
            			function(response){
