...
var failureFun = function (json) {
	if (Ext.isEmpty(json)) {
		miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.requestTimeOut')); //请求超时
	} else {
...
Ext.define('Miser.priceCustomer.statusStore', {
    ...
    data : [
		          //生效中
		          {'name':pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.statusEffective'),'value':'EFFECTIVE'},
		            
                  //已过期
                  {'name':pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.statusPassed'),'value':'PASSED'},
                  
                   //待生效
                  {'name':pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.statusWait'),'value':'WAIT'},
                  
                   //已废弃
                  {'name':pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.statusDeleted'),'value':'DELETED'}
              ]
});
...
function statusTypeRender(value) {
	...
	if ('PASSED' == value) {
		value = pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.statusPassed');
	} else if ('EFFECTIVE' == value) {
		value = pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.statusEffective');
	} else if ('WAIT' == value) {
		value = pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.statusWait');
	} else if ('DELETED' == value) {
		value = pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.statusDeleted');
	}
	return value;
}
...
/**
 * 客户专属价格 model 对应数据表 t_price_customer_section_sub
 */
Ext.define('Miser.model.priceCustomerSubEntity', {
	...
	,{
		...
		type:'string'
//		defaultValue:'60000000000000000001'
	}
	,{
...
//		defaultValue:'易-入户'
		...
		name:'firstWeight',//一段首重
		type:'string'
	},{
		name:'firstWeightPrice',//一段首重单价
		type:'string'
	},{
		name:'firstAddWeightPrice',//一段续重单价
		type:'string'
	},{
		name:'secondWeight',//二段首重
		type:'string'
	},{
		name:'secondWeightPrice',//二段首重单价
		type:'string'
	},{
		name:'secondAddWeightPrice',//二段续重单价
		type:'string'
	},{
		name:'thirdWeight',//三段首重
		type:'string'
	},{
		name:'thirdWeightPrice',//三段首重单价
		type:'string'
	},{
		name:'thirdAddWeightPrice',//三段续重单价
		type:'string'
	},{
		name : 'remark',// 备注
		type : 'string'
		...
		name : 'freightSectionCode',// 运费优惠分段
		type : 'string'
	},{
		name:'freightSectionCodeName',//运费优惠分段名称
		type:'string'
	}]
});
...
Ext.define('Miser.store.priceCustomerStore',{
			...
			proxy : {
				type : 'ajax',
				actionMethods : 'post',
				url : '../pricecard/priceCustomerSection!listCustomerInfo.action',
				reader : {
					type : 'json',
					rootProperty : 'priceCustomerSectionVo.priceCunsomerSectionList',
			...
			listeners : {
				beforeload : function(store, operation, eOpts) {
					...
					if (queryForm != null) {
						...
						var params = {
								'priceCustomerSectionVo.priceCustomerSectionEntity.divisionCode':queryForm.getForm().findField('divisionCode').getValue(),
								'priceCustomerSectionVo.priceCustomerSectionEntity.bigRegionCode':queryForm.getForm().findField('bigRegionCode').getValue(),
								'priceCustomerSectionVo.priceCustomerSectionEntity.roadAreaCode':queryForm.getForm().findField('roadAreaCode').getValue(),
								'priceCustomerSectionVo.priceCustomerSectionEntity.saleDepartCode':queryForm.getForm().findField('saleDepartCode').getValue(),
								'priceCustomerSectionVo.priceCustomerSectionEntity.customerCode':queryForm.getForm().findField('customerCode').getValue(),
								'priceCustomerSectionVo.priceCustomerSectionEntity.customerName':queryForm.getForm().findField('customerName').getValue(),
								'priceCustomerSectionVo.priceCustomerSectionEntity.status':queryForm.getForm().findField('status').getValue(),
								'priceCustomerSectionVo.priceCustomerSectionEntity.effectiveTime':queryForm.getForm().findField('effectiveTime').getValue()
						};
...
Ext.define('Miser.store.priceCustomerSubStore', {
	...
	proxy : {
		type : 'ajax',
		actionMethods : 'post',
		url : 'priceCustomerSection!queryCustomerSubListByParam.action',
		reader : {
			type : 'json',
			rootProperty : 'priceCustomerSectionSubVo.priceCustomerSubEntityList',
			totalProperty : 'totalCount' // 总个数
		}
	},
	listeners : {
		beforeload : function(store, operation, eOpts) {
			var parentId = operation.priceCustomerSectionId;
			if(parentId != ''  ||  parentId != undefined)
			{
				var params = {
						'priceCustomerSectionSubVo.priceCustomerSubEntity.parentId' :parentId
				};
				Ext.apply(store.proxy.extraParams, params);
			}
		}
...
Ext.define(
			'Miser.view.bseCustomer.QueryForm',
			{
				...
				title : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.queryCondition'),// '查询条件',
				collapsible : true,
				region : 'north',
				defaults : {
					labelWidth : 0.3,
				...
				constructor : function(config) {
					var me = this, cfg = Ext.apply({}, config);
							me.items = [
									{
										...
										items : [{
								            ...
								            //事业部
								            divisionLabel: pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.divisionCodeLabel')+':',
								            ...
								            // 大区
								            bigregionLabel: pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.bigRegionCodeLabel')+':',
								            ...
								            // 路区
								            roadareaLabel: pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.roadAreaCodeLabel')+':',
								            ...
								            // 门店
								            salesdepartmentLabel: pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.saleDepartCodeLabel')+':',
									...
									},
									{
										...
										items : [ {
											name : 'customerCode',
											fieldLabel :pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.customerCodeLabel') ,//'客户编号'
											xtype : 'textfield',
											margin:'5px 0px 5px 0px',
											labelAlign:'right'
										},{
											name : 'customerName',
											fieldLabel : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.customerNameLabel'),//'客户名称'
											...
											name : 'status',
											fieldLabel : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.curStatusLabel'),//'当前状态'
											xtype : 'priceCustomer_status_combselector',
											margin:'5px 0px 5px 0px',
											labelAlign:'right'
										},{
							    			name: 'effectiveTime',
							    			fieldLabel: pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.effectiveTimeLabel'),//'有效时间点'
							    			xtype : 'datetimefield',
							    			format : 'Y-m-d H:i:s',
							    			margin:'5px 0px 5px 0px',
							    			labelAlign:'right'
							...
							me.buttons = [
									{
										text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.query'),//查询
										handler : function() {
											...
											else
											{
												...
												if(customerNameVal != null && customerNameVal != '' ){
													...
													if(divisionCodeVal == null || divisionCodeVal == ''){
														miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.divisionCodeQryCtrl'));//请选择所属事业部后查询
														return;
													}
													
													//所有模糊查询需要要求输入4个字符以上才能查询，否则提示：模糊查询必须输入4个字符以上才能查询
													if( strlen(customerNameVal) < 4){
														miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.likeQryMoreFourChars'));//"模糊查询必须输入4个字符以上才能查询"
														return;
													}												
												}						
											}
									...
									{
										text :pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.reset'),//重置
										handler : function() {
											me.getForm().reset();
										}
									} ];
...
Ext.define(
				'Miser.view.priceCustomer.Grid',
				{
					...
					stripeRows : true,
					selType : "rowmodel",
					emptyText : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.qryResultIsNull'), //'查询结果为空'
					columnLines : true,
					viewConfig : {
					...
					constructor : function(config) {
						...
						me.columns = [
								{
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.sequenceNo'),//'序号'
									width : 60,
									xtype : 'rownumberer',
									align : 'center'
								},
								...
								{
									 //扩展 所属事业部名称
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.divisionCodeLabel'),
									width : 100,
								...
								{
									//扩展 所属大区名称
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.bigRegionCodeLabel'),
									width : 150,
								...
								{
									//扩展 所属路区名称
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.roadAreaCodeLabel'),
									width : 100,
								...
								{
									//扩展 所属门店名称
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.saleDepartCodeLabel'),
									width : 200,
								...
								{
									//物流代码
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.logistCode'),
									width : 100,
								...
								},
								{
									...
									width : 100,
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.customerCodeLabel')
								},
								{
									dataIndex : 'customerName',
									width : 200,
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.customerNameLabel'),
									align : 'center'
								},
							    {
									//生效时间  
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.effectiveTimeLabel2'),
							    ...
							    {
									//失效时间
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.invalidTimeLabel2'),
							    ...
							    {
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.statusLabel'),//状态
									dataIndex : 'status',
									width : 100,
									align : 'center',
									renderer : function(value) {
								...
								{
									text :pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.remarkLabel') ,//'备注'
									dataIndex : 'remark',
									width : 100,
									align : 'center'
								},
								{
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.createTimeLabel'),//'创建时间',
								...
								{
									text :pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.createUserNameLabel'),// '创建人',
									dataIndex : 'createUserName',
									width : 140,
									align : 'center'
								},
								{
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.modifyUserNameLabel'),//'修改人',
								...
								{
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.modifyTimeLabel'),//'修改时间',
						...
						me.tbar = [
								{
									id : 'miser_biz_pricecard_priceCustomerSection_AddBtn',
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.addBtn'),//'新增',
									xtype : 'addbutton',
									width : 80,
									handler : function() {
								...
								},
								{
									...
									text :pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.updateBtn'),//'修改',
									disabled : true,
									xtype : 'updatebutton',
									width : 80,
									handler : function() {
										...
										var params = {
												'priceCustomerSectionVo':{
													'priceCustomerSectionEntity':{
														'id':id
													}
												}
										};
										...
										var successFun = function (json) {
											...
											//后台返回的组织实体
											var priceCustomerEntity = json.priceCustomerSectionVo.priceCustomerSectionEntity;
										...
										miser.requestJsonAjax('../pricecard/priceCustomerSection!queryPriceCustomerAndSubList.action', params, successFun, failureFun);								
									}
								},
								{
									id : 'miser_biz_pricecard_priceCustomer_DelBtn',
									text :pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.deleteBtn'),// '作废',
									xtype : 'deletebutton',
									disabled : true,
									width : 80,
									handler : function() {
										...
										if(isExistDiffStatus){
											//'请选择同种状态的数据进行该项操作'
							    			miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.selectDiffStatusRecordToOper'));
							    			return;
										}
										else{
													if(destoryIdStr != '')
													{
														miser.showQuestionMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.destoryConfirmTipMsg'),
														                     function(e) {
																				if (e == 'yes') {
																					...
																					var successFun = function (json) {
																						...
																						if(msg == "miser_priceCustomer_StatusChange"){
																							...
																							//'状态已发生变化，请刷新后重新操作'
																							miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.statusHasChanged'));
																						}
																						else if(msg == "miser_priceCustomer_destorySuccess"){
																							
																							//提示消息 "作废成功"
																							miser.showInfoMsg(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.destroySuccess'));
																					...
																					miser.requestJsonAjax('../pricecard/priceCustomerSection!destoryPriceCustomerAndSubList.action', params, successFun, failureFun);		
																				}
																		});
													}
											   }
								...
								},
								{
									...
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.viewBtnLabel'),//'查看详情',
									xtype : 'searchbutton',
									disabled : true,
									width : 100,
									handler : function() {
									...
									text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.exprortResultList'),//'导出结果列表',
									...
									handler : function() {
										...
										Ext.Ajax.request( {  
										       url : '../pricecard/priceCustomerSection!excelExportCustomer.action',  
										       method : 'post',
										       timeout:180000,
										       params : params,  
						...
						me.items = [
								...
								},
								{
									...
									items : [ {
						    			...
						    			name: 'effectiveTime',
						    			fieldLabel: pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.effectiveTimeLabel'),//'有效时间',
						    			xtype : 'datetimefield',
						    			format : 'Y-m-d H:i:s',
						    			allowBlank:false,
						    			labelAlign:'right'
						    			...
						    			name: 'invalidTime',
						    			fieldLabel: pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.invalidTimeLabel'),//'失效时间',
						    			xtype : 'datetimefield',
						    			format : 'Y-m-d H:i:s',
						    			allowBlank:false,
						    			labelAlign:'right'
...
//									items:[
//										 {
//											 fieldLabel : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.isNewCustomerLabel'),//是否使用新价格城市
//											 name:'customerType',
...
//								},
								{
									...
									items : [ 
									 {
										fieldLabel : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.remarkLabel'),//'备注',
										name : 'remark',
...
//	title : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.addCustomerWinTitle'),//'新增客户专属价格',
	title : '新增易到家客户价格',//'新增客户专属价格',
	closable : true,
	parent : null,
	modal : true,
	resizable : false,
	...
	listeners : {
		beforehide : function(me) { // 隐藏WINDOW的时候清除数据
			...
			Ext.getCmp('searchArrivePriceCityForAdd').setValue(null);
...
//			customerTypeObj.setReadOnly(false);
	...
	submitPriceCustomerAddForm : function() {
		...
		if (me.getPriceCustomerAddForm().getForm().isValid()) { // 校验form是否通过校验
			...
			curData.customerName =  me.getPriceCustomerAddForm().getForm().findField('customerName').rawValue.trim();
			
			if(curData.invalidTime <= curData.effectiveTime)
			{
				//'失效时间 必须大于 生效时间'
				miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.invalidTimeThanEffectiveTime'));
				return;
			}
			...
			var params = {
					'priceCustomerSectionVo' : {
						'priceCustomerSectionEntity' : curData,
						'priceCustomerSectionSubEntityList' : priceCustomerSubEntityList
					}
				};
			...
			var failureFun = function(json) {
				...
				if (Ext.isEmpty(json)) {
					miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.requestTimeOut'));
				} else {
					var message = json.message;
					showPriceCustomerTipMsg(message);
				}
			};
			miser.requestJsonAjax('priceCustomerSection!addPriceCustomer.action',params, successFun, failureFun); // 发送AJAX请求
		}
		else {
	...
	constructor : function(config) {
		var me = this, cfg = Ext.apply({}, config);
		me.fbar = [ {
			id:'searchArrivePriceCityForAdd',
				//fieldLabel:pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.arrivePriceCity'),
				xtype:'textfield'
			},{
				text:'搜索',
				handler : function(){
					var subViewGrid = me.getPriceCustomerSubAddGrid();
					subCustomerDetailSearch(subViewGrid,'searchArrivePriceCityForAdd');
				}
			},{
				xtype:'tbfill'
			},{
			text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.saveBtn'),//'保存',
			handler : function() {
				me.submitPriceCustomerAddForm();
			}
		},{
			text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.cancelBtn'),//'取消',
			handler : function() {
				me.close();
			}
		}];
...
//									items:[
//										{
//										    fieldLabel : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.isNewCustomerLabel'),//是否新客户
//										    name:'customerType',
//										    xtype: 'yesnocombselector',
//										    isShowAll : false,
//										    labelAlign:'right',
...
Ext.define('Miser.view.pricecard.priceCustomerUpdateWindow', {
	extend : 'Ext.window.Window',
//	title : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.modifyCustomerWinTitle'),//'修改客户价格',
	title : '修改易到家客户价格',//'修改易到家客户价格',
	closable : true,
	parent : null,
	modal : true,
	resizable : false,
...
//				customerTypeObj.setReadOnly(true);
			...
			me.getPriceCustomerSubAddGrid().store.reload({
				'priceCustomerSectionId':me.priceCustomerEntity.id
			});
	...
	submitPriceCustomerUpdateForm : function() {
		...
		if (me.getPriceCustomerUpdateForm().getForm().isValid()) { // 校验form是否通过校验
			...
			miser.requestJsonAjax('priceCustomerSection!updatePriceCustomer.action',params, successFun, failureFun); // 发送AJAX请求
		}
		else {
			miser.showErrorMes('表单校验不通过,请检查是否填写完整');
		}
...
Ext.define('Miser.view.pricecard.priceCustomerViewWindow', {
	extend : 'Ext.window.Window',
//	title : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.viewWindowTitle'),//'客户专属价格详情',
	title : '易到家客户价格详情',//'易到家客户专属价格详情',
	closable : true,
	parent : null,
	modal : true,
	resizable : false,
...
//				customerTypeObj.setValue(me.priceCustomerEntity.customerType);
				...
				var textFieldObjArr = [divisionObj,bigRegionObj,roadArea,saleDepart,customerCodeObj,customerNameObj,effectiveTimeObj,invalidTimeObj,remarkObj];
	...
	constructor : function(config) {
		...
		me.fbar = [ {
			...
			handler : function() {
				...
				var successFun = function (json) {
					//bugfix by heyu 查看窗口点击新增时会new一个新的表单，在调用rowEdit的时候会到之前的那个表单，导致出现编辑失效，现在改为弹出之前的表单，不new一个新的
					var addGrid = Ext.getCmp('priceCustomerGrid');
					var bseCustomerEntityList = json.bseCustomerVo.bseCustomerEntityList;
					var bseCustomerEntity = bseCustomerEntityList[0];
					addGrid.getPriceCustomerAddWindow(cfg).setBseCustomerEntity(null);
					addGrid.getPriceCustomerAddWindow(cfg).setBseCustomerEntity(bseCustomerEntity);
...
//					me.getPriceCustomerAddWindow(cfg).setBseCustomerEntity(bseCustomerEntity);
					...
					addGrid.getPriceCustomerAddWindow(cfg).show();
				};
			...
			handler : function() {			
				...
				var successFun = function (json) {
					//bugfix by heyu 查看窗口点击修改时会new一个新的表单，在调用rowEdit的时候会到之前的那个表单，导致出现编辑失效，现在改为弹出之前的表单，不new一个新的
					var updateGrid = Ext.getCmp('priceCustomerGrid');
...
//					me.getPriceCustomerUpdateWindow(cfg).getPriceCustomerUpdateForm().id = 'priceCustomerViewUpdateForm';
					...
					//后台返回的组织实体
					var priceCustomerEntity = json.priceCustomerSectionVo.priceCustomerSectionEntity;
					updateGrid.getPriceCustomerUpdateWindow(cfg).setPriceCustomerEntity(null);
					updateGrid.getPriceCustomerUpdateWindow(cfg).setPriceCustomerEntity(priceCustomerEntity);
//					me.getPriceCustomerUpdateWindow(cfg).setPriceCustomerEntity(null);
//					me.getPriceCustomerUpdateWindow(cfg).setPriceCustomerEntity(priceCustomerEntity);
...
//				var customerType = viewForm.getForm().findField('customerType').getValue();
			...
			text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.closeBtn'),//'关闭',
			handler : function() {
				me.close();
			}
		}];
...
var rowEditing = Ext.create('Ext.grid.plugin.RowEditing', {
    ...
    saveBtnText:pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.saveBtn'),//'保存',
    cancelBtnText:pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.cancelBtn'),//'取消',
    errorSummary:false,
    listeners:{
    	edit:function(e,o){
...
//    		}
    		...
    		//运费优惠分段
    		if(data.freightSectionCodeName !=""){
    			Ext.getCmp('freightFeeSection_add').setCombValue(data.freightSectionCodeName,data.freightFeeSection);	
    		}    		
    		return true;
    	},
...
//    		//运输类型
    		...
    		if(data.freightSectionCodeName !=""){
    			Ext.getCmp('freightFeeSection_update').setCombValue(data.freightSectionCodeName,data.freightFeeSection);	
    		}
    		return true;
    	},
		dblclick: function(e,o){
		...
		cancelEdit:function (e,o){
			...
			//e.grid.store.removeAt(o.rowIdx);
			var sto = e.grid.store.getAt(o.rowIdx);
			...
			me.columns = [
					{
						...
						width : 100,
						text :pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.transTypeCode'),//'运输类型',
						editor: {
...
//							readOnly:true,
					...
					},
					{
						...
						width : 120,
					    text:pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.sendPriceCity'),//'出发价格城市',
					    editor:{
					...
					},
					{
						...
						width:120,
						text:pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.arrivePriceCity'),//'到达价格城市',
						editor:{
...
//								 }
					...
					},
					{
						...
						width : 120,
//						text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.weightPrice'),//'重量单价',
						text : '首重(1段)',//'首重(1段)',
			            editor: {
			            	id:'firstWeight_add',
			                allowBlank: false,
			                ...
			                validator: function () { 
			                    ...
			                    if(value>0){
			                    	var secondWeight = Ext.getCmp('secondWeight_add').getValue();
				                    var thirdWeight = Ext.getCmp('thirdWeight_add').getValue();
				                    var firstWeightPrice = Ext.getCmp('firstWeightPrice_add').getValue();
				                    var firstAddWeightPrice = Ext.getCmp('firstAddWeightPrice_add').getValue();
			                    	if(secondWeight>0 && value>=secondWeight){
				                    	return "首重（1段）应小于首重（2段）";
				                    }
				                    else if(thirdWeight>0 && value>=thirdWeight){
				                    	return "首重（1段）应小于首重（3段）";
				                    }
			                    	if(firstWeightPrice==null || firstWeightPrice==0){
			                    		return "首重（1段）大于0，请输入首重单价和续重单价";
			                    	}
			                    	if(firstAddWeightPrice==null || firstAddWeightPrice==0){
			                    		return "首重（1段）大于0，请输入首重单价和续重单价";
			                    	}
			                    }
			                    	return true; 
					...
					},
					{
						...
						width : 120,
//						text :pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.volumePrice'),// '体积单价',
						text :'首重(1段)单价',// '首重(1段)单价',
			            editor: {
			            	id:'firstWeightPrice_add',
			                allowBlank: false,
							minValue: 0,
							decimalPrecision:2,
			                xtype: 'numberfield',
			                validator: function () { 
			                	var firstWeight = Ext.getCmp('firstWeight_add');
			                    var value = this.getValue();
			                    if(firstWeight.getValue()>0){
			                    	if(value==null || value==0){
			                    		return "首重大于0，请输入首重单价";
			                    	}
			                    	if(firstWeight.isValid()){
			                    		firstWeight.clearInvalid();
			                    	}
			                    }
			                    return true;
			                }    
...
//						text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.addFee'),//'附加费',
						text : '续重(1段)单价',//'续重(1段)单价',
						width : 120,
			            editor: {
			            	id:'firstAddWeightPrice_add',
			            	allowBlank: false,
							minValue: 0,
							decimalPrecision:2,
			                xtype: 'numberfield',
...
//						text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.lowestFee'),//'最低收费',
						text : '首重(2段)',//'首重(2段)',
						width : 120,
			            editor: {
			            	id:'secondWeight_add',
//			            	allowBlank: false,
			                ...
			                validator: function () {  
			                    var value = this.getValue();
			                    if(value>0){
			                    	var firstWeight = Ext.getCmp('firstWeight_add').getValue();
				                    var thirdWeight = Ext.getCmp('thirdWeight_add').getValue();
				                    var secondWeightPrice = Ext.getCmp('secondWeightPrice_add').getValue();
				                    var secondAddWeightPrice = Ext.getCmp('secondAddWeightPrice_add').getValue();
				                    if(firstWeight>0 && value<=firstWeight){
				                    	return "首重（2段）应大于首重（1段）";
				                    }
				                    else if(thirdWeight>0 && value>=thirdWeight){
				                    	return "首重（2段）应小于首重（3段）";
				                    }
				                    if(secondWeightPrice==null || secondWeightPrice==0){
			                    		return "首重（2段）大于0，请输入首重单价和续重单价";
			                    	}
			                    	if(secondAddWeightPrice==null || secondAddWeightPrice==0){
			                    		return "首重（2段）大于0，请输入首重单价和续重单价";
			                    	}
			                    }
			                    	return true; 
...
//						text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.lowestFee'),//'最低收费',
						text : '首重(2段)单价',//'首重(2段)单价',
						width : 120,
			            editor: {
//			            	allowBlank: false,
			            	id:'secondWeightPrice_add',
							minValue: 0,
							decimalPrecision:2,
			                xtype: 'numberfield',
			                validator: function () { 
			                	var secondWeight = Ext.getCmp('secondWeight_add');
			                    var value = this.getValue();
			                    if(secondWeight.getValue()>0){
			                    	if(value==null || value==0){
			                    		return "首重大于0，请输入首重单价";
			                    	}
			                    	if(secondWeight.isValid()){
			                    		secondWeight.clearInvalid();
			                    	}
			                    }
			                    return true;
			                }    
...
//						text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.lowestFee'),//'最低收费',
						text : '续重(2段)单价',//'续重(2段)单价',
						width : 120,
			            editor: {
//			            	allowBlank: false,
			            	id:'secondAddWeightPrice_add',
							minValue: 0,
							decimalPrecision:2,
			                xtype: 'numberfield',
			                validator: function () { 
...
//			            	allowBlank: false,
			                ...
			                validator: function () {  
			                    ...
			                    if(value>0){
			                    	var firstWeight = Ext.getCmp('firstWeight_add').getValue();
				                    var secondWeight = Ext.getCmp('secondWeight_add').getValue();
				                    var thirdWeightPrice = Ext.getCmp('thirdWeightPrice_add').getValue();
				                    var thirdAddWeightPrice = Ext.getCmp('thirdAddWeightPrice_add').getValue();
				                    if(firstWeight>0 && value<=firstWeight){
				                    	return "首重（3段）应大于首重（1段）";
				                    }
				                    else if(secondWeight>0 && value<=secondWeight){
				                    	return "首重（3段）应大于首重（2段）";
				                    }
				                    if(thirdWeightPrice==null || thirdWeightPrice==0){
			                    		return "首重（3段）大于0，请输入首重单价和续重单价";
			                    	}
			                    	if(thirdAddWeightPrice==null || thirdAddWeightPrice==0){
			                    		return "首重（3段）大于0，请输入首重单价和续重单价";
			                    	}
			                    }   
			                    	return true; 
...
//						text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.lowestFee'),//'最低收费',
						text : '首重(3段)单价',//'首重(3段)单价',
						width : 120,
			            editor: {
//			            	allowBlank: false,
			            	id:'thirdWeightPrice_add',
							minValue: 0,
							decimalPrecision:2,
			                xtype: 'numberfield',
			                validator: function () { 
			                	var thirdWeight = Ext.getCmp('thirdWeight_add');
			                    var value = this.getValue();
			                    if(thirdWeight.getValue()>0){
			                    	if(value==null || value==0){
			                    		return "首重大于0，请输入首重(3段)单价";
			                    	}
			                    	if(thirdWeight.isValid()){
			                    		thirdWeight.clearInvalid();
			                    	}
			                    }
			                    return true;
			                }    
...
//						text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.lowestFee'),//'最低收费',
						text : '续重(3段)单价',//'续重(3段)单价',
						width : 120,
			            editor: {
//			            	allowBlank: false,
			            	id:'thirdAddWeightPrice_add',
							minValue: 0,
							decimalPrecision:2,
			                xtype: 'numberfield',
			                validator: function () { 
					...
					},
					{
						...
						text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.freightFeeSection'),//'运费优惠分段',
						width : 120,
						editor: {
						...
						renderer: function(value,o,m){
							  ...
							  if(sto != null && sto != undefined && sto.getStore() !=null && sto.getStore().data.length != 0)
							  {
								  display = getDisplayTextCommon(sto.getStore(),value);
								  if(display == value){
									  display = m.data.freightSectionCodeName;
								  }
							  }
							  else
							 {
							  ...
							  m.data.freightSectionCodeName = display;
							 return m.data.freightSectionCodeName;
						 }
					}];
...
//						updateForm.getForm().findField('customerType').setValue('Y');								
				...
				},'-',
				{
					...
					width : 130,
					handler:function(){
						miser.requestExportAjax('/excelTemplate/priceCustomerSection.xls');
					}
				}];
					me.selModel = Ext.create('Ext.selection.CheckboxModel',{
										listeners : {
...
//							text : pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.weightPrice'),//'重量单价',
				            ...
				            editor: {
				                ...
				                validator: function () { 
				                	var value = this.getValue();
				                    if(value>0){
				                    	var secondWeight = Ext.getCmp('secondWeight_update').getValue();
					                    var thirdWeight = Ext.getCmp('thirdWeight_update').getValue();
					                    var firstWeightPrice = Ext.getCmp('firstWeightPrice_update').getValue();
					                    var firstAddWeightPrice = Ext.getCmp('firstAddWeightPrice_update').getValue();
				                    	if(secondWeight>0 && value>=secondWeight){
						                    	return "首重（1段）应小于首重（2段）";
						                 }
						                 else if(thirdWeight>0 && value>=thirdWeight){
...
//							text :pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.volumePrice'),// '体积单价',
				            ...
				            editor: {
				                allowBlank: false,
				                id:'firstWeightPrice_update',
								minValue: 0,
								decimalPrecision:2,
				                xtype: 'numberfield',
				                validator: function () { 
				                	var firstWeight = Ext.getCmp('firstWeight_update');
				                    var value = this.getValue();
				                    if(firstWeight.getValue()>0){
				                    	if(value==null || value==0){
				                    		return "首重大于0，请输入首重(1段)单价";
				                    	}
				                    	if(firstWeight.isValid()){
				                    		firstWeight.clearInvalid();
				                    	}
...
//				            	allowBlank: false,
				                ...
				                validator: function () {  
				                    var value = this.getValue();
				                    if(value>0){
				                    	var firstWeight = Ext.getCmp('firstWeight_update').getValue();
					                    var thirdWeight = Ext.getCmp('thirdWeight_update').getValue();
					                    var secondWeightPrice = Ext.getCmp('secondWeightPrice_update').getValue();
					                    var secondAddWeightPrice = Ext.getCmp('secondAddWeightPrice_update').getValue();
					                    if(firstWeight>0 && value<=firstWeight){
					                    	return "首重（2段）应大于首重（1段）";
					                    }
					                    else if(thirdWeight>0 && value>=thirdWeight){
						...
						{						
				            ...
				            editor: {
//				            	allowBlank: false,
				            	if:'secondWeightPrice_update',
								minValue: 0,
								decimalPrecision:2,
				                xtype: 'numberfield',
				                validator: function () { 
				                	var secondWeight = Ext.getCmp('secondWeight_update');
				                    var value = this.getValue();
				                    if(secondWeight.getValue()>0){
				                    	if(value==null || value==0){
				                    		return "首重大于0，请输入续重(2段)单价";
						...
						{						
				            ...
				            editor: {
//				            	allowBlank: false,
				            	id:'secondAddWeightPrice_update',
								minValue: 0,
								decimalPrecision:2,
				                xtype: 'numberfield',
				                validator: function () { 
				                    ...
				                    if(secondWeight.getValue()>0){
				                    	...
				                    	if(secondWeight.isValid()){
							                ...
							                rowEditing.startEdit(linenum,6);
				                    		secondWeight.clearInvalid();
				                    	}
				                    }
				                    return true;
...
//				            	allowBlank: false,
				                ...
				                validator: function () {  
				                    ...
				                    if(value>0){
				                    	var firstWeight = Ext.getCmp('firstWeight_update').getValue();
					                    var secondWeight = Ext.getCmp('secondWeight_update').getValue();
					                    var thirdWeightPrice = Ext.getCmp('thirdWeightPrice_update').getValue();
					                    var thirdAddWeightPrice = Ext.getCmp('thirdAddWeightPrice_update').getValue();
					                    if(firstWeight>0 && value<=firstWeight){
					                    	return "首重（3段）应大于首重（1段）";
					                    }
					                    else if(secondWeight>0 && value<=secondWeight){
						...
						{						
				            ...
				            editor: {
//				            	allowBlank: false,
				            	id:'thirdWeightPrice_update',
								minValue: 0,
								decimalPrecision:2,
				                xtype: 'numberfield',
				                validator: function () { 
				                	var thirdWeight = Ext.getCmp('thirdWeight_update');
				                    var value = this.getValue();
				                    if(thirdWeight.getValue()>0){
				                    	if(value==null || value==0){
				                    		return "首重大于0，请输入续重(3段)单价";
...
//		}
		...
		Ext.Msg.wait('处理中，请稍后...', '提示');
		Ext.Ajax.request( {  
	       url : '../pricecard/priceCustomerSection!excelImpl.action',
	       method : 'post',  
	       params : {  
	    	   'priceCustomerSectionVo.filePath' : outFileName  
	          },  
	          success : function(response, options) {  
	              Ext.Msg.hide();   
	              var responseArray = Ext.util.JSON.decode(response.responseText);
...
function down(id,exportType,customerType)
{
	Ext.Msg.wait('处理中，请稍后...', '提示');
	var params={
		'priceCustomerSectionVo.priceCustomerSectionEntity.id':id,
		'priceCustomerSectionVo.exportType':exportType
	 };
	 Ext.Ajax.request( {  
	       url : '../pricecard/priceCustomerSection!excelExport.action',  
	       method : 'post',  
	       params : params,  
	          success : function(response, options) {  
	             //隐藏进度条   
...
function showPriceCustomerTipMsg(message){
	...
	{
		//生效时间必须晚于当前时间一小时
		miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.effectiveAfterOneHour'));
	}
	else if(message == 'priceCustomer.priceDiscountExistRepeatTimezone')
	{
		//"该客户已有正在生效的专属价格，且有效时间重合，不能提交！"
		miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.validRecrodExistRepeatTime'));
	}
	else if(message == 'priceCustomer.EffectiveTimeDoesNotBetweenContractStartEndTime'){
		//生效时间必须在客户合同的开始时间和结束时间区间内
		miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.EffectiveTimeDoesNotBetweenContractStartEndTime'));
	}
	else if(message == 'priceCustomer.ContractStartOrEndTimeInvalid'){
		//客户合同的开始时间或结束时间为空不合法
		miser.showErrorMes(pricecard.priceCustomerSection.i18n('priceCustomer.ContractStartOrEndTimeInvalid'));
	}
	else if(message == 'priceCustomer.subDetailSendPriceCityNameCannotNull'){
		//出发价格城市名称不能为空
		miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.subDetailIsInvalid'));
	}
		...
		miser.showErrorMes(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.passedInvalidTimeMustThanCurTime'));
	}	
	else
	{
		miser.showErrorMes(message);
...
function priceCustomerSubDetailCheck(priceCustomerSubEntityItems){
	...
	if(priceCustomerSubEntityItems.length == 0){
		miser.showInfoMsg(pricecard.priceCustomerSection.i18n('pricecard.priceCustomer.subDetailCannotBeNull'));
		return;
	}
...
function subCustomerDetailSearch(grid,searchId){
    ...
    //用于存放需要搜索的字段
	var keyArr = ['sendPriceCityName','arrivePriceCityName','transTypeName','freightSectionCodeName'];
