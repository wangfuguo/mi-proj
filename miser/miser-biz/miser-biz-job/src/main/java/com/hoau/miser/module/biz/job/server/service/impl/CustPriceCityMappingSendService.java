...
import com.hoau.miser.module.biz.job.server.dao.CustPriceCityMappingSendDao;
import com.hoau.miser.module.biz.job.server.service.ICustPriceCityMappingSendService;
...
/**
 * 价格城市关系映射Service
 * ClassName: PriceCityMappingSendService 
 * @author 廖文强
 * @date 2016年2月28日
 * @version V1.0
 */
@Service
public class CustPriceCityMappingSendService implements
		ICustPriceCityMappingSendService {
	@Resource
	...
	@Resource
	private CustPriceCityMappingSendDao custPriceCityMappingSendDao;
	/**
	...
	static{
		try {
			...
			String maxCountStr = properties.getProperty("SENDMQ.PRICECITYMAPPING.MAXCOUNT", "100");
	...
	public void send() {
		...
		DataSendJobTimeEntity resultEntity = dataSendJobTimeService
				.getLastSendTime(DataSendJobConstant.JOB_BIZ_SEND_CUST_PRICE_CITY_MAPPING);
		// 组装查询条件
		...
		queryFailureFilterEntity.setType(DataSendJobConstant.JOB_BIZ_SEND_CUST_PRICE_CITY_MAPPING);
		//失败信息去重
		custPriceCityMappingSendDao.failureInfoUniq(queryFailureFilterEntity);
		//发送失败信息
		
		//发送上次失败数据
		ArrayList<PriceCityMappingInfo> failureDatas= custPriceCityMappingSendDao.queryFailureData(queryFailureFilterEntity);
		sendData(failureDatas, true);
		//发送本次数据
		ArrayList<PriceCityMappingInfo> needDatas=custPriceCityMappingSendDao.queryNeedSendData(queryNewDataFitlerEntity);
		sendData(needDatas, false);	
		// 更新上次发送时间
		DataSendJobTimeEntity saveEntity = new DataSendJobTimeEntity();
		saveEntity.setId(UUIDUtil.getUUID());
		saveEntity.setJobCode(DataSendJobConstant.JOB_BIZ_SEND_CUST_PRICE_CITY_MAPPING);
		saveEntity.setJobName(DataSendJobConstant.getNameByCode(DataSendJobConstant.JOB_BIZ_SEND_CUST_PRICE_CITY_MAPPING));
	 ...
	 */
	@Transactional
	public void sendData(ArrayList<PriceCityMappingInfo> datas, boolean isFailureData) {
		//activeMQProducerService.postPriceCityMappingToQueue();
		ArrayList<PriceCityMappingInfo> priceCityMappingData = new ArrayList<PriceCityMappingInfo>(MAX_COUNT);
		for (int i = 0; i < datas.size(); i++) {
			...
			if ((i + 1) % MAX_COUNT == 0 || i == datas.size()-1) {
				try {
					activeMQProducerService.postCustomerPriceCityMappingDataToQueue(priceCityMappingData);
					if (isFailureData) {
						//如果是失败数据发送成功了更新失败表状态为已发送
						for (int j = 0; j < priceCityMappingData.size(); j++) {
							PriceCityMappingInfo failureEntity = datas.get((i / MAX_COUNT) * MAX_COUNT + j); //获取此次发送的数据
					...
					if (!isFailureData) {
						...
						for (int j = 0; j < priceCityMappingData.size(); j++) {
							...
							saveFailureEntity.setType(DataSendJobConstant.JOB_BIZ_SEND_CUST_PRICE_CITY_MAPPING);
